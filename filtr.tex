...
\author{Rajnil Malviya\\EE23BTECH11051}
\date{}
...
range for our bandpass filter as $5.8$ kHz - $7$ kHz.  Hence, the un-normalized discrete time filter
...
\begin{align}
    F_{s1} &= 5.8-0.3 = 5.5 \text{KHz}\\
    F	_{s2} &= 7+0.3 = 7.3  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.229 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.304 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.3959$, $\Omega_{p2} = 0.489$ and $\Omega_{s1} = 0.376$, $\Omega_{s2} = 0.517$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.439$ and $B = \Omega_{p2} - \Omega_{p1} = 0.0931$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.456\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.558
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.456.
\end{align}
...
\begin{lstlisting}
https://github.com/rajnilm/filterdesign/blob/main/codes/p1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/f1.png}
\caption{The Analog Low-Pass Frequency Response for $0.278 \leq \epsilon \leq 0.619$}
...
\begin{lstlisting}
https://github.com/rajnilm/filterdesign/blob/main/codes/c1.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/rajnilm/filterdesign/blob/main/codes/p2.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.227$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.227}{\brak{s_L-s_5}\brak{s_L-s_6}\brak{s_L-s_7}\brak{s_L-s_8}}\\
     &= \frac{0.227}{s_{L}^4 + 0.9052s_{L}^3 +1.4096s_{L}^2 +0.6936  s_{L} +0.2593}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{1.52\times 10^{-4}s^4}{s^8 + 0.179s^7 + 0.811s^6 + 0.107s^5 + 0.240s^4 + 0.021s^3 + 0.030s^2 + 0.001s + 0.001} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{G(z^8 - 4z^6 + 6z^4 - 4z^2 + 1)}{2.39195z^8 - 12.3537z^7 + 32.72278z^6 - 54.77282z^5 + 63.1198z^4 - 50.8227z^3 + 28.1737z^2 - 9.86902z + 1.77353)}
\end{align}

where $G=1.52\times 10^{-4}$    
\begin{figure}[H]
\centering
...
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/f2.png}
